...
const MenuLeft = () => {
    ...
    const { preActiveSelection, setPreActiveSelection, menuActive, setMenuActive, MenuData } = context;
    ...
    useEffect(() => {
        ...
        const next_ele = mainMenuRef.current[menuActive];
        const overly_ele = menuOverlayRef.current;
        if (!pre_ele || !next_ele || !overly_ele) return;
    ...
    }, [menuActive, preActiveSelection, setPreActiveSelection]);

    const mainMenuClickHandler = (index: number) => {
        if (index === menuActive) return;
        setMenuActive(index);
    }
                ...
                {MenuData.slice(0, 5).map((item, index) => {
                    return (<div key={index} onClick={() => mainMenuClickHandler(index)} ref={(el) => { mainMenuRef.current[index] = el; }} className="flex z-1 w-full cursor-pointer grow-0 shrink-0 items-center p-[0.5vw] rounded-[0.8vw] gap-[1vw] border-2 border-[#d5dce249] ">
                        <div className="bg-[#fce0b6] rounded-[0.4vw]">
                            <Image src={item.svg} className="w-[2.8vw] h-[2.8vw] p-[0.4vw]" alt="burger_icon" width={64} height={64} />
                        </div>
