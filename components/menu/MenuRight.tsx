...
const MenuRight = () => {
  ...
  // Context
  const { itemActive, setItemActive, menuActive, setMenuActive, MenuItemData, setMenuItemData } = useContext(BurgerMenuContext);
        ...
        {item !== null && (
          <div className=" w-full h-full flex flex-col justify-between  gap-[0.6vw] p-[1vw] pb-0">
            ...
            <div className="w-full mb-1 h-fit flex justify-between mt-auto">
              ...
              {!status
                ?
                <button onClick={() => cartBtnHandler()} className="w-[40%] h-full cursor-pointer bg-[#FC9412] py-[0.2vw] rounded-[0.8vw] font-bold text-[1.2vw]"> Add-to-Cart</button>
                :
                <Link className="w-[40%] h-full cursor-pointer bg-[#FC9412] py-[0.2vw] text-center rounded-[0.8vw] font-bold text-[1.2vw]" href="/chackout">Check-Out</Link>
              }
            </div>
          </div>
        )}
